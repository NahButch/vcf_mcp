...
your own machine and communicates with your MCP client (e.g. Claude Desktop)
...
- **No data collection by the author.** The author/maintainer of vcf-mcp
  ...
  tracking.
- **Your genomic data never leaves your machine.** VCF file contents are read
  from local disk, queried in memory, and returned only to your local MCP
  client.
...
|---|---|---|
| VCF file **contents** (variants, genotypes, etc.) | Local files you register | Read into memory to answer queries; results returned to your local MCP client only |
| Tabix / rsID **indexes** | Built in memory from your VCF files | Held in RAM for the life of the server process; never written to your source folder |
| **Registry metadata**: sample name, file path, genome build, and your optional description | Provided by you via `add_sample` / config | Persisted to a local state file (see below) |

vcf-mcp does **not** read, infer, or store any data beyond what is needed to
answer the queries you make.

## What is stored on disk, and where

The only thing vcf-mcp writes to disk is a small **state file** recording the
samples you have registered, so they survive a restart. It contains, per
sample: the **name**, the **absolute file path**, the **genome build**, and
your optional **description**. It does **not** contain genomic variant data.
...
vcf-mcp **never** writes a `.tbi` index, or any other file, next to your source
VCFs. Index construction happens in memory.

## Data sharing

vcf-mcp shares data with **no one**. The only recipient of any output is the
local MCP client you connected it to, on the same machine. What that client
...
generating a response — review your client's privacy terms to understand that
boundary.

## Data retention and deletion

- **Indexes/caches** live only in RAM and vanish when the server process exits.
- **Registry metadata** persists in the state file until you remove it:
...
- vcf-mcp never deletes or modifies your underlying VCF files.
...
that flag, it can register any readable file path you provide. It only ever
reads files you explicitly register or that are found by an
`add_samples_from_folder` scan you initiate.
